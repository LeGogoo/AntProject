...
public class AntManager : MonoBehaviour
{
    ...
    private void OnCollisionEnter(Collision collision)
    {
        if (collision.gameObject.layer == LayerMask.NameToLayer("Wall"))
        {
            ...
            var dis = transform.forward + contact.normal;
            m_curVelocity = (contact.normal + dis).normalized * velocity;
            Vector3 lookAtPosition = transform.position + m_curVelocity;
    ...
    private IEnumerator RotateTransFormNextFrame(Quaternion q)
    {
        ...
        yield return null;
        m_curVelocity = transform.forward * velocity;
    }
    ...
    private void CheckWall()
    {
        foreach(var trans in CheckPointList)
        {
            ...
            {
                var dis = transform.forward + hit.normal;
